var decisionTree = function () {
    ...
    var fill = ["#ff425d"];                          // parents nodes colors
    var fillLeaves = "#5ca6ba";                      // leaf nodes color
    ...
    function createGraph(jsonData, options) {
        ...
        var svgPathLabel;                            // label "Predict Path"
        ...
        function smallNode() {
            ...
            d3.select("#" + containerId).attr("style", "width: "
                ...
                "border-radius: 4px; overflow-x: hidden; overflow-y: hidden; position: relative; float: left;"
            );
        ...
        function appendButtons() {
            $("#" + containerId).append(
                ...
                + "<img src='../src/images/download.png' id='saveImg' title='保存图片' class='treebtn treebtn5'></button> "
                + "</div>"
            );
        }
